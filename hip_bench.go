...
type HipParams struct {
	...
	CA1Size      evec.Vec2i `desc:"size of CA1"`
...
type Sim struct {
	...
	//TE           TEParams                    `desc:"parameters for the testing effect"`
	PoolVocab map[string]*etensor.Float32 `view:"no-inline" desc:"pool patterns vocabulary"`
	Tables    map[string]*etable.Table
	// OverlapTrain    *etable.Table `view:"no-inline" desc:"patterns to use - training overlapping routes"`
	// OverlapPFCTrain *etable.Table `view:"no-inline" desc:"patterns to use - training overlapping routes with PFC inhib"`
	//TrainRP      *etable.Table            `view:"no-inline" desc:"patterns to use - retrieval practice"`
	//TrainRS      *etable.Table            `view:"no-inline" desc:"patterns to use - restudy"`
	TrainRoute *etable.Table `view:"no-inline" desc:"patterns to use - training nonoverlapping routes"`
	TestRoute  *etable.Table `view:"no-inline" desc:"patterns to use - training nonoverlapping routes"`
	// NonOverlapTrain *etable.Table            `view:"no-inline" desc:"patterns to use - training nonoverlapping routes"`
	// TrainAC         *etable.Table            `view:"no-inline" desc:"AC training patterns to use"`
	// OverlapTest     *etable.Table            `view:"no-inline" desc:"patterns to use - testing overlapping routes"`
	// OverlapPFCTest  *etable.Table            `view:"no-inline" desc:"patterns to use - testing overlapping routes with PFC"`
	// NonOverlapTest  *etable.Table            `view:"no-inline" desc:"patterns to use - testing nonoverlapping routes"`
	...
	HiddenReps   map[string]*Reps         `view:"no-inline" desc:"representational analysis of Hidden layer, sorted by relationship"`
	...
	TrgOnWasOffOverlap  float64 `inactive:"+" desc:"current trial's proportion of bits where target = on but ECout was off ( < 0.5); semantic only"`
	TrgOnWasOffDistinct float64 `inactive:"+" desc:"current trial's proportion of bits where target = on but ECout was off ( < 0.5); Vid only"`
	TrgOffWasOnOverlap  float64 `inactive:"+" desc:"current trial's proportion of bits where target = off but ECout was on ( > 0.5); house only"`
	TrgOffWasOnDistinct float64 `inactive:"+" desc:"current trial's proportion of bits where target = off but ECout was on ( > 0.5); Vid only"`
...
func (ss *Sim) New() {
	...
	ss.MemThr = 0.34
	ss.LayStatNms = []string{"ECout", "ECin", "DG", "CA3", "CA1"}
	ss.TstRecLays = []string{"ECout", "ECin", "DG", "CA3", "CA1"}
	ss.HiddenReps = make(map[string]*Reps)
	for _, lnm := range ss.TstRecLays {
		reps := &Reps{}
		reps.Init()
		ss.HiddenReps[lnm] = reps
	}
	ss.TstNms = []string{"TestRoute"}
	ss.TstStatNms = []string{"Mem", "TrgOffWasOn", "TrgOnWasOff", "TrgOffWasOnOverlap", "TrgOnWasOffOverlap"}
	ss.SimMatStats = []string{"Same", "Overlap", "NonOverlap"}
...
func (hp *HipParams) Defaults() {
	...
	// bigger size - save for running on hpc2, too slow for my comp
	hp.ECSize.Set(2, 3)
	hp.ECPool.Set(16, 16)
	hp.CA1Size.Set(2, 3)
	hp.CA1Pool.Set(20, 20) // big hip
	hp.CA3Size.Set(40, 40) // big hip
	hp.DGRatio = 2.236     // sqroot 5

	// ratio
	hp.DGPCon = 0.15 // .35 is sig worse, .2 learns faster but AB recall is worse
	hp.CA3PCon = 0.15
...
func (ss *Sim) ConfigEnv() {
	...
	ss.TrainEnv.Table = etable.NewIdxView(ss.TrainRoute)
	...
	ss.TestEnv.Table = etable.NewIdxView(ss.TestRoute)
...
// note: may need to include retrieval practice here
func (ss *Sim) SetEnv(routetrain bool) {
	if routetrain {
		ss.TrainEnv.Table = etable.NewIdxView(ss.TrainRoute)
	} else {
		ss.TrainEnv.Table = etable.NewIdxView(ss.TrainRoute)
...
func (ss *Sim) ConfigNet(net *leabra.Network) {
	...
	pfc := net.AddLayer4D("PFC", hp.CA1Size.Y, hp.CA1Size.X, 1, 1, emer.Input)
	...
	pfc.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "ECout", YAlign: relpos.Front, Space: 2})
	...
	// PFC
	pj = net.ConnectLayers(pfc, ca1, pool1to1, emer.Inhib)
	pj.SetClass("PFCInhib")
	pj = net.ConnectLayers(pfc, ecin, pool1to1, emer.Inhib)
	pj.SetClass("PFCInhib")
...
func (ss *Sim) AlphaCyc(train bool) {
	...
	//pfc := ss.Net.LayerByName("PFC").(leabra.LeabraLayer).AsLeabra()
		...
		case 3: // Fourth Quarter: CA1 back to ECin drive only
			...
			if chg {
				...
				if epc < ss.MaxEpcs/2 {
					...
					ecinFmPfc.WtScale.Abs = 5
					ca1FmPfc.WtScale.Abs = 5
				}
			}
			ss.Net.GScaleFmAvgAct() // update computed scaling factors
			ss.Net.InitGInc()       // scaling params change, so need to recompute all netins
...
// ClearOffInputs - clears mask if input is missing
// func (ss *Sim) ClearOffInputs() {
// 	input := ss.Net.LayerByName("Input").(leabra.LeabraLayer).AsLeabra()
// 	ecout := ss.Net.LayerByName("ECout").(leabra.LeabraLayer).AsLeabra()
// 	msk := bitflag.Mask32(int(leabra.NeurHasExt), int(leabra.NeurHasTarg), int(leabra.NeurHasCmpr))
// 	for ni := range input.Neurons {
// 		nrn := &ecout.Neurons[ni]
// 		n := &input.Neurons[ni]
// 		pl := &input.Pools[n.SubPool]
// 		if pl.ActM.Avg < 0.05 {
// 			nrn.ClearMask(msk)
// 		}
// 	}
...
func (ss *Sim) InitStats() {
	...
	ss.TrgOnWasOffAll = 0
	ss.TrgOnWasOffCmp = 0
	ss.TrgOnWasOffOverlap = 0
	ss.TrgOnWasOffDistinct = 0
	ss.TrgOffWasOn = 0
	ss.TrgOffWasOnOverlap = 0
	ss.TrgOffWasOnDistinct = 0
	ss.TrlSSE = 0
	ss.TrlAvgSSE = 0
	ss.EpcSSE = 0
	ss.EpcAvgSSE = 0
...
func (ss *Sim) MemStats(train bool) {
	...
	trgOffOverlap := 0.0
	trgOffDistinct := 0.0
	trgOffWasOnOverlap := 0.0
	trgOffWasOnDistinct := 0.0
	trgOnWasOffOverlap := 0.0
	trgOnWasOffDistinct := 0.0
	actMi, _ := ecout.UnitVarIdx("ActM")
	targi, _ := ecout.UnitVarIdx("Targ")
	actQ1i, _ := ecout.UnitVarIdx("ActQ1")
	ecPoolSize := ss.Hip.ECPool.X * ss.Hip.ECPool.Y // ecPool expanded
	overlapPoolsStart := 1                          // last item pool
	overlapPoolsEnd := 3
	distinctPoolsStart := 3 // last semantic pool
	distinctPoolsEnd := 5
	for ni := 0; ni < nn; ni++ {
	...
	for ni := overlapPoolsStart*ecPoolSize - 1; ni < overlapPoolsEnd*ecPoolSize; ni++ {
		actm := ecout.UnitVal1D(actMi, ni)
		trg := ecout.UnitVal1D(targi, ni) // full pattern target
		inact := ecin.UnitVal1D(actQ1i, ni)
		if trg < 0.5 { // trgOff
	...
	for ni := distinctPoolsStart*ecPoolSize - 1; ni < distinctPoolsEnd*ecPoolSize; ni++ {
		actm := ecout.UnitVal1D(actMi, ni)
		trg := ecout.UnitVal1D(targi, ni) // full pattern target
		inact := ecin.UnitVal1D(actQ1i, ni)
		if trg < 0.5 { // trgOff
	...
	trgOffWasOn /= trgOffN
	trgOnWasOffOverlap /= trgOnOverlap
	trgOffWasOnOverlap /= trgOffOverlap
	trgOnWasOffDistinct /= trgOnDistinct
	trgOffWasOnDistinct /= trgOffDistinct
	if train { // no cmp
		...
		if cmpN > 0 {
			trgOnWasOffCmp /= cmpN
			if trgOnWasOffCmp < ss.MemThr && trgOffWasOn < ss.MemThr { // trgOnWasOffOverlap - overlapping features only; trgOnWasOffCmp - overlapping and distinct features
				ss.Mem = 1
			} else {
				ss.Mem = 0
			}
	...
	ss.TrgOffWasOnOverlap = trgOffWasOnOverlap
	ss.TrgOnWasOffOverlap = trgOnWasOffOverlap
	ss.TrgOffWasOnDistinct = trgOffWasOnDistinct
	ss.TrgOnWasOffDistinct = trgOnWasOffDistinct
}
...
func (ss *Sim) TrialStats(accum bool) (sse, avgsse, cosdiff float64) {
	...
	ss.TrlSSE, ss.TrlAvgSSE = outLay.MSE(0) // 0.5 = per-unit tolerance -- right side of .5
	if accum {
		ss.SumSSE += ss.TrlSSE
...
func (ss *Sim) ConfigPats() {
	...
	patgen.AddVocabEmpty(ss.PoolVocab, "overlap1", npats, plY, plX)
	patgen.AddVocabEmpty(ss.PoolVocab, "overlap2", npats, plY, plX)
	patgen.AddVocabPermutedBinary(ss.PoolVocab, "distinct1", npats, plY, plX, pctAct, minDiff)
	patgen.AddVocabPermutedBinary(ss.PoolVocab, "distinct2", npats, plY, plX, pctAct, minDiff)
	patgen.AddVocabPermutedBinary(ss.PoolVocab, "destination1", npats, plY, plX, pctAct, minDiff)
	patgen.AddVocabPermutedBinary(ss.PoolVocab, "destination2", npats, plY, plX, pctAct, minDiff)

	for i := 0; i < npats/2; i++ {
		for j := 0; j < npats/2; j++ {
			ss.PoolVocab["overlap1"].SubSpace([]int{j + i*npats/2}).CopyFrom(ss.PoolVocab["overlap1_tmp"].SubSpace([]int{i}))
			ss.PoolVocab["overlap2"].SubSpace([]int{j + i*npats/2}).CopyFrom(ss.PoolVocab["overlap2_tmp"].SubSpace([]int{i}))
		}
	}
	...
	for i := 0; i < 2; i++ {
		overlapstrengthnm := fmt.Sprintf("overlap%d", i+1)
		tsr, _ := patgen.AddVocabClone(ss.PoolVocab, overlapstrengthnm, overlapstrengthnm)
...
func (ss *Sim) LogTrnTrl(dt *etable.Table) {
	...
	for _, lnm := range ss.LayStatNms {
		tsr := ss.ValsTsr(lnm)
		ly := ss.Net.LayerByName(lnm).(leabra.LeabraLayer).AsLeabra()
		ly.UnitValsTensor(tsr, "Act") // get minus phase act
...
func (ss *Sim) LogTstTrl(dt *etable.Table) {
	...
	row := dt.Rows
	if ss.TestNm == "TestRoute" && trl == 0 { // reset at start
		row = 0
	}
	dt.SetNumRows(row + 1)
	...
	dt.SetCellFloat("TrgOffWasOn", row, ss.TrgOffWasOn)
	dt.SetCellFloat("TrgOffWasOnOverlap", row, ss.TrgOffWasOnOverlap)
	dt.SetCellFloat("TrgOffWasOnDistinct", row, ss.TrgOffWasOnDistinct)
	dt.SetCellFloat("TrgOnWasOffOverlap", row, ss.TrgOnWasOffOverlap)
	dt.SetCellFloat("TrgOnWasOffDistinct", row, ss.TrgOnWasOffDistinct)
...
func (ss *Sim) ConfigTstTrlLog(dt *etable.Table) {
	...
	sch := etable.Schema{
		...
		{"TrgOnWasOff", etensor.FLOAT64, nil, nil},
		{"TrgOffWasOn", etensor.FLOAT64, nil, nil},
		{"TrgOnWasOffOverlap", etensor.FLOAT64, nil, nil},
		{"TrgOffWasOnOverlap", etensor.FLOAT64, nil, nil},
		{"TrgOnWasOffDistinct", etensor.FLOAT64, nil, nil},
		{"TrgOffWasOnDistinct", etensor.FLOAT64, nil, nil},
	}
	for _, lnm := range ss.LayStatNms {
...
func (ss *Sim) ConfigTstTrlPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	...
	// sets defaults
	// plt.SetColParams("Mem", eplot.On, eplot.FixMin, 0, eplot.FixMax, 1)
	plt.SetColParams("TrgOnWasOffOverlap", eplot.On, eplot.FixMin, 0, eplot.FixMax, 1)
	plt.SetColParams("TrgOffWasOnOverlap", eplot.On, eplot.FixMin, 0, eplot.FixMax, 1)
	plt.SetColParams("TrgOnWasOffDistinct", eplot.On, eplot.FixMin, 0, eplot.FixMax, 1)
	plt.SetColParams("TrgOffWasOnDistinct", eplot.On, eplot.FixMin, 0, eplot.FixMax, 1)
...
// note: need to fix so that we can look at first train/test, retrieval practice, and second train/test
// note: need to fix so that we can look at first train/test, retrieval practice, and second train/test
func (ss *Sim) SimMatStat(lnm string) (float64, float64, float64) {
	...
	same_sum := float64(0)
	same_n := 0
	overlap_sum := float64(0)
	overlap_n := 0
	nonoverlap_sum := float64(0)
	nonoverlap_n := 0
	for y := 0; y < nitm; y++ {
		for x := 0; x <= y; x++ {
			val := smat.FloatVal([]int{y, x}) // r score
			same := y == x                    // i.e., on diag
			if same {
				same_sum += val
				same_n++
			} else if (x+y == 1) || (x+y == 5) { // overlapping
				overlap_sum += val
				overlap_n++
			} else {
				nonoverlap_sum += val
	...
	if same_n > 0 {
		same_sum /= float64(same_n)
	}
	if overlap_n > 0 {
		overlap_sum /= float64(overlap_n)
	}
	if nonoverlap_n > 0 {
		nonoverlap_sum /= float64(nonoverlap_n)
	}
	return same_sum, overlap_sum, nonoverlap_sum
}
...
func (ss *Sim) LogTstEpc(dt *etable.Table) {
	...
	if ss.LastEpcTime.IsZero() {
		...
		nt := ss.TrainRoute.Rows * 4 // 1 train and 3 tests
	...
	for _, lnm := range ss.LayStatNms {
		same_sum, overlap_sum, nonoverlap_sum := ss.SimMatStat(lnm)
		for _, ts := range ss.SimMatStats {
			switch ts {
			case "Same":
				dt.SetCellFloat(lnm+" "+ts, row, same_sum)
			case "Overlap":
				dt.SetCellFloat(lnm+" "+ts, row, overlap_sum)
			case "NonOverlap":
				dt.SetCellFloat(lnm+" "+ts, row, nonoverlap_sum)
			}
		}
	}

	// base zero on testing performance!
	curAB := ss.TrainEnv.Table.Table == ss.TrainRoute
	var mem float64
	if curAB {
		mem = dt.CellFloat("Overlap Mem", row)
		mem = dt.CellFloat("Overlap Mem", row)
	} else {
		mem = dt.CellFloat("NonOverlap Mem", row)
		mem = dt.CellFloat("NonOverlap Mem", row)
	}
...
func (ss *Sim) ConfigRunStatsPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	...
	cp := plt.SetColParams("TestRoute Mem:Mean", eplot.On, eplot.FixMin, 0, eplot.FixMax, 1)
	cp.ErrCol = "TestRoute Mem:Sem"
	// cp = plt.SetColParams("Weak Mem:Mean", eplot.On, eplot.FixMin, 0, eplot.FixMax, 1)
	// cp.ErrCol = "Weak Mem:Sem"
...
// IsRPLoopParams are the parameters to run for testing effect conditions
// var IsRPLoopParams = []string{"RP", "RS"}

// // FourFactorRun runs outer-loop crossed with inner-loop params, as well as two added Testing Effect loops
// func (ss *Sim) FourFactorRun() {
// 	tag := ss.Tag
// 	usetag := tag
// 	if usetag != "" {
// 		usetag += "_"
// 	}
// 	for _, rprs := range IsRPLoopParams {
// 		ss.Tag = usetag + rprs
// 		//rand.Seed(ss.RndSeed + int64(ss.BatchRun))
